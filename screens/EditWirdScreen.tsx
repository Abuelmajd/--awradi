...
import { CalendarIcon, PersonLinkedIcon, BookOpenIcon, CalendarDaysIcon } from '../components/Icons';

interface EditWirdScreenProps {
  navigate: (page: Page, params?: any) => void;
  wird: Wird;
  onUpdateWird: (wird: Wird) => void;
  onDeleteWird: (wirdId: number) => void;
}

const EditWirdScreen: React.FC<EditWirdScreenProps> = ({ navigate, wird, onUpdateWird, onDeleteWird }) => {
  const [wirdName, setWirdName] = useState(wird.name);
  const [wirdType, setWirdType] = useState<WirdType>(wird.type);
  const [linkedPrayer, setLinkedPrayer] = useState<PrayerName>(wird.linkedPrayer || 'Fajr');
  const [specificDays, setSpecificDays] = useState<Set<number>>(new Set(wird.specificDays || []));
  const [target, setTarget] = useState(wird.target);
  ...
  const handleSave = () => {
    ...
    onUpdateWird(updatedWird);
    navigate('awrad');
  };
  
  const handleDelete = () => {
      if (window.confirm(`هل أنت متأكد من حذف ورد "${wird.name}"؟`)) {
          onDeleteWird(wird.id);
          navigate('awrad');
      }
  }
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-900">
      <Header title="تعديل الورد" onBack={() => navigate('wird-detail', { wird })} />
